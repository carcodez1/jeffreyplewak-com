...
const inputDir = path.resolve(repoRoot, env("DIAGRAMS_IN", "src/content/diagrams"));
const outputDir = path.resolve(repoRoot, env("DIAGRAMS_OUT", "public/diagrams"));
...
for (const file of files) {
  ...
  const cmd = `npx -y @mermaid-js/mermaid-cli mmdc -i "${input}" -o "${output}"`;
